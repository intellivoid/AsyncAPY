...
@server.add_handler(Filters.Fields(foo=r"test_payload"))
# This is checked before the echo handler
async def filtered_handler(client, packet):
    print(f"Hello 1 from {client}!")
    print(f"Packet received is {packet}")
    await client.send(
        Packet({"response": "OK"}, encoding=client.encoding)
    )  # You can choose an encoding
    await client.close()


@server.add_handler(group=-1)
# Will be executed before any other handler because -1 < 0
